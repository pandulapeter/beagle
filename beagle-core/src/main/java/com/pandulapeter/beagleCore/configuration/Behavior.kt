 ...
 * @param shouldShowResetPendingChangesButton - Whether or not to display a Reset button besides the Apply button that appears when the user makes changes that are not handled in real-time (see the "needsConfirmation" parameter of some Tricks). True by default.
 * @param shouldResetPendingChangesOnClose - Whether or not to reset all pending changes when the drawer is closed (see the "needsConfirmation" parameter of some Tricks). False by default.
...
data class Behavior(
    ...
    val shouldShowResetPendingChangesButton: Boolean = true,
    val shouldResetPendingChangesOnClose: Boolean = false,
